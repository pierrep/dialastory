...
def upload_all(server, 
    ...
    if local_files:
        ...
        pkey1 = paramiko.RSAKey.from_private_key_file('./penguin_rsa.key')

        try:
            transport = paramiko.Transport(('54.206.48.162', 22))
            transport.connect(username='booth', pkey=pkey1)
        ...
        if server_connect_ok:
            ...
            if login_ok:
                ...
                for file_info in local_files:
                    ...
                    if(rexists(sftp,'upload/'+filename)):
                        print filename+"\tfile exists already..."
                    else:
                        print filename + " file doesn't exist. Uploading!"
                        sftp.put('mp3s/'+filename, 'upload/'+filename);
